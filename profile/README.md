# Verana

**The Open Trust Layer for the Internet**  
Turn digital services into verifiable assets that anyone can find and trust.

## 🌍 What is Verana?

Verana is a **public, decentralized trust infrastructure** that enables ecosystems, organizations, and individuals to build verifiable services and networks they truly **own**.  

We solve three systemic problems of the internet:  

- **Trust Crisis** → no cryptographic identity built in  
- **Ownership Illusion** → platforms control your data and audience  
- **Discovery Monopoly** → visibility controlled by ads & algorithms  

With Verana, you get:  

- 🔐 **Cryptographic Trust** — every interaction is verifiable and authentic  
- 🗝 **True Ownership** — you control your services, data, and audience  
- 🧭 **Fair Discovery** — be found for what you prove, not what you pay  

## 📚 Learn More

- [Documentation](https://docs.verana.io) — APIs, SDKs, tutorials  
- [Verifiable Trust Specification](https://verana-labs.github.io/verifiable-trust-spec/) — open standard for verifiable services  
- [Verifiable Public Registry Specification](https://verana-labs.github.io/verifiable-trust-vpr-spec/) — decentralized trust registries  
- [Verana Foundation](https://veranafoundation.org) — the nonprofit stewarding the project  
- [Verana Council](https://veranacouncil.org) — governance & decision-making body  
- [Verana Infrastructure](https://verana.io) — the public trust infrastructure  

## 🤝 Join the Community

- 💬 [Discord](https://discord.gg/edjaFn252q) — developer chat & Q&A  
- 🧑‍💻 [GitHub](https://github.com/verana-labs) — contribute to open source  
- 🔗 [LinkedIn](https://www.linkedin.com/company/verana-foundation) — updates & thought leadership  
- 📰 [X (Twitter)](https://x.com/Verana_io) — news & announcements  
